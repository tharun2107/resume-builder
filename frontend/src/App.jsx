  ...
  return (
    <div className="app-container" style={{ minHeight: '100vh', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: 'linear-gradient(135deg, #e0e7ff 0%, #f7f7fa 100%)' }}>
      ...
      <div style={{ width: '420px', background: '#fff', borderRadius: '12px', boxShadow: '0 2px 16px #e0e7ff', padding: '2rem', margin: '2rem 1rem', minHeight: '80vh', overflowY: 'auto', maxHeight: '90vh' }}>
        ...
        {/* Removed outer <form> to avoid nested forms */}
        <fieldset style={{ border: 'none', marginBottom: '1.5rem' }}>
        ...
        {/* SectionList components for Experience, Projects, Education, Certifications, Custom Sections remain unchanged */}
        <SectionList sectionKey="experience" title="Experience" items={resume.experience} onAdd={(item) => addSectionItem("experience", item)} onEdit={(idx, item) => editSectionItem("experience", idx, item)} onDelete={(idx) => deleteSectionItem("experience", idx)} addLabel="Add Experience" emptyLabel="No experience added." renderItem={(item = {}, onChange) => onChange ? (<><input name="company" placeholder="Company" value={item.company || ""} onChange={onChange} style={{ padding: '6px' }} /><input name="role" placeholder="Role" value={item.role || ""} onChange={onChange} style={{ padding: '6px' }} /><input name="duration" placeholder="Duration" value={item.duration || ""} onChange={onChange} style={{ padding: '6px' }} /><textarea name="details" placeholder="Details (comma separated)" value={item.details || ""} onChange={onChange} style={{ padding: '6px' }} /></>) : (<><span style={label}>{item.role}</span> at <span style={label}>{item.company}</span><span style={dateStyle}>{item.duration}</span><ul style={bulletList}>{(item.details || '').split(',').map((d, i) => d.trim() && <li key={i} style={bullet}>{d.trim()}</li>)}</ul></>)} />
        <SectionList sectionKey="projects" title="Projects" items={resume.projects} onAdd={(item) => addSectionItem("projects", item)} onEdit={(idx, item) => editSectionItem("projects", idx, item)} onDelete={(idx) => deleteSectionItem("projects", idx)} addLabel="Add Project" emptyLabel="No projects added." renderItem={(item = {}, onChange) => onChange ? (<><input name="title" placeholder="Title" value={item.title || ""} onChange={onChange} style={{ padding: '6px' }} /><input name="link" placeholder="Link" value={item.link || ""} onChange={onChange} style={{ padding: '6px' }} /><textarea name="description" placeholder="Description (comma separated for bullets)" value={item.description || ""} onChange={onChange} style={{ padding: '6px' }} /></>) : (<><span style={label}>{item.title}</span> {item.link && (<a href={item.link} target="_blank" rel="noopener noreferrer" style={{ color: '#3b5bdb', marginLeft: 4, fontSize: '0.95em' }}>[link]</a>)}<ul style={bulletList}>{(item.description || '').split(',').map((d, i) => d.trim() && <li key={i} style={bullet}>{d.trim()}</li>)}</ul></>)} />
